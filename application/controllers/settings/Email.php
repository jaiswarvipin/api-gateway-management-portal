...
class Email extends Requestprocess {
	...
	public function setEmailDetails(){
		...
		$strEmailDetailsDataArr	= $this->_objDataOperation->getDataFromTable(array('table'=>$this->_strPrimaryTableName, 'where'=>$strWhereClauseArr));
		
		/* if module already exists then do needful */
		if(!empty($strEmailDetailsDataArr)){
			/* Return Information */
