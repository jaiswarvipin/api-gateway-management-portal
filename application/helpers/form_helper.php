...
class Form{
	...
	/*Inputs	: $pStrDropDownElementArr :: drop down elements,
				...
				: $pBlnDonotEncryptKey :: Default do not encrypt the options key value
	/*Returns 	: Drop Down HTML.
	/*Created By: Jaiswar Vipin Kumar R.
	/**********************************************************************/
	function getDropDown($pStrDropDownElementArr = array(), $pStrSelecedValue = '', $pBlnSelectOne = true, $pBlnDonotEncryptKey = false){
		...
		foreach($pStrDropDownElementArr as $pStrDropDownElementArrKey => $pStrDropDownElementArrValue){
			...
			if(is_array($pStrDropDownElementArrValue)){
				...
				$strValue	= $pStrDropDownElementArrKey;
				/* if default encryption is off then do needful */
				if(!$pBlnDonotEncryptKey){
					/* Setting the encrypted value */
					$strValue	= getEncyptionValue($pStrDropDownElementArrKey);
				}
				/* Setting the option array */
				$strDropDownHTML .= '<option value="'.$strValue.'" '.$strSelected.'>'.$pStrDropDownElementArrValue.'</option>';
			}
		}
