	        	...
	        	<?php if(!empty($dataSet)){
					...
					foreach($dataSet as $dataSetKey => $dataSetValue){?>
						<tr>
		            		...
		            		<td>
		            			<a href="javascript:void(0);" onclick="openEditModel('<?php echo $strDataAddEditPanel?>','<?php echo getEncyptionValue($dataSetValue['id'])?>',1);" title="Click here to manage the Module Access to this Role" class="waves-effect waves-circle waves-light secondary-content"><i class="material-icons">edit</i></a>
							...
							<?php foreach($strModuleArr as $strModuleArrKey => $strModuleArrValue){
									if(isset($strModuleArrValue['description'])){?>
										<tr>
											...
											<td><label><input class='validate' type='checkbox' name='txtModulename[]' id='txtModulename<?php echo getEncyptionValue($strModuleArrValue['id'])?>' value="<?php echo getEncyptionValue($strModuleArrValue['id'])?>" data-set="module_code" />&nbsp;<span for="txtModulename<?php echo getEncyptionValue($strModuleArrValue['id'])?>">&nbsp;</span></label></td>
										</tr>
								<?php }?>
								<?php if(isset($strModuleArrValue['child'])){?>
									<?php foreach($strModuleArrValue['child'] as $strModuleArrValueKey => $strModuleArrValueDetails){?>
										<tr>
											...
											<td><label><input class='validate' type='checkbox' name='txtModulename[]' id='txtModulename<?php echo getEncyptionValue($strModuleArrValueDetails['id'])?>' value="<?php echo getEncyptionValue($strModuleArrValueDetails['id'])?>" data-set="module_code" />&nbsp;<span for="txtModulename<?php echo getEncyptionValue($strModuleArrValueDetails['id'])?>">&nbsp;</span></label></td>
										</tr>
									<?php }?>
								<?php }?>
							<?php }?>
