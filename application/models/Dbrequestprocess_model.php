...
class Dbrequestprocess_model extends CI_Model{
	...
	public function getDirectQueryResult($pStrQuery = ''){
		...
		if(strstr(strtolower($pStrQuery), 'insert ')){
			...
			/* Return the last inserted code */
			$strReturnValue	= $this->db->insert_id();
		}else if((strstr(strtolower($pStrQuery), 'select ')) || (strstr(strtolower($pStrQuery), 'show '))){
			/* execute direct query */
	...
	/********************************************************************/
	/* Purpose 		: Insert mass update.
	/* Inputs		: $pStrDataArr	= Data set.
	/* Returns 		: Updated status.
	/* Created By 	: Jaiswar Vipin Kumar R.
	/********************************************************************/
	public function setBulkInset($pStrDataArr = array()){
		/* Variable initialization */
		$intInsertRecord = 0;
		...
		$this->db->insert_batch($pStrDataArr['table'], $pStrDataArr['data']);

		/* printing query log */
		$this->_getlastQuery();

		/* Checking for number of record updated */
		$intInsertRecord	= $this->db->affected_rows();
		...
		if((isset($pStrFilterArr['table'])) && (!empty($pStrFilterArr['table']))){
			...
			if(is_array($pStrFilterArr['table'])){
				...
				foreach($pStrFilterArr['table'] as $pStrFilterArrKey => $pStrFilterArrValue){
					...
					/* checking table is not consider in the ignore deleted record clause */
					if(!isset($pStrFilterArr['ignoreDelete'][$pStrFilterArrValue])){
						/* Setting filter */
			...
			/* for single table */
				...
				/* checking table is not consider in the ignore deleted record clause */
				if(!isset($pStrFilterArr['ignoreDelete'][$pStrFilterArr['table']])){
					/* Setting filter */
	...
	public function isTableExists($pStrTabelName = ''){
		...
		if((trim($pStrTabelName) == '') || ((int)$this->db->table_exists($pStrTabelName) == 0)){
			/* printing query log */
			$this->_getlastQuery();
			return false;
		}else{
